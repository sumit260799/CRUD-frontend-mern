...
function UpdateUser() {
  ...
  const { EditUser, updateItem } = useGlobalContext();
  const { id } = useParams();
  const defaultVal = {
    fname: "",
    lname: "",
  ...
  const { fname, lname, password, city, state, zip } = fields;

  const handleSubmit = async (e) => {
    e.preventDefault();

    await EditUser(id, fields);
    navigate("/alluser");
  };
  ...
  return (
    <form className="w-full mx-auto my-12 max-w-lg" onSubmit={handleSubmit}>
